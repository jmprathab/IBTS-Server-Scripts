...
if (isset($_POST['name']) && isset($_POST['mobile']) && isset($_POST['password'])) {
    ...
    $selectUser = 'match(user:User{mobile:{mobile},password:{password}}) return user';
    $parameters = array('mobile' => $mobileNumber, 'password' => $password);
    //Creating New User
    $name=ucwords($name);
    $createUser = 'create(user:User{name:{name},mobile:{mobile},password:{password},balance:{balance},time:timestamp()}) return user';
    $parameters = array('name' => $name, 'mobile' => $mobileNumber, 'password' => $password,'balance'=>0.0);
